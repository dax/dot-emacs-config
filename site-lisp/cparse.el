...
;;; Version: $Id: cparse.el,v 1.1 2001/08/05 21:43:03 mandor Exp $
;;; Keywords: c
;;;
...
(defun cparse-line2vect ()
  ...
  (let ((l cparse-mainlist) (n 0))
    (save-excursion
      ...
      (beginning-of-line)
      (re-search-forward "\t\\([0-9]+\\)" nil t)
      (setq n (string-to-number (cparse-bss (match-beginning 1)
                                            (match-end 1))))
...
(defun cparse-fast-find (file)
  ...
  (let ((b nil)
	(c-mode-hook nil)
	(find-file-hook cparse-brief-find-file-hooks))
